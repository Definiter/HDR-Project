...
% % Wait until the shutter button is pressed. To record the initial image.
% while true
...
%     end
% end
% pause(shutter_speed + 1)
% beep
% 
% % Wait until the shutter button is pressed. To record the image with no filter.
% while true
%     button = screencapture(rect_button_center);
%     button_pressed = mean(button(:)) > 128;
%     if button_pressed
